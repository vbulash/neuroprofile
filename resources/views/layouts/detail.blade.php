...
@section('buttons')
	...
	<div class="col-sm-5">
		...
		@else
			...
			<a class="btn btn-secondary pl-3" href="@yield('form.close')" role="button">Закрыть</a>
		@endif
	</div>
@endsection
...
@section('interior')
	...
	<form role="form" method="post" @yield('form.params') autocomplete="off" enctype="multipart/form-data">
		@csrf
								...
								@case('textarea')
									<div class="col-sm-5">
										<textarea class="form-control" name="{{ $field['name'] }}" id="{{ $field['name'] }}" cols="30" rows="5"
										 @if ($mode == config('global.show') || isset($field['disabled'])) disabled @endif>{{ isset($field['value']) ? old($field['name'], $field['value']) : old($field['name']) }}</textarea>
									</div>
								@break
								...
								@case('radio')
									<div class="col-sm-5">
										<div class="form-group btn-group col-lg-3 col-xs-12" role="group">
											...
											@endphp
											@foreach ($field['options'] as $key => $value)
												<input type="radio" class="btn-check" name="{{ $field['name'] }}" id="{{ $field['name'] . $item }}"
													autocomplete="off" @if ($mode == config('global.create')) @if ($loop->first) checked @endif
												@elseif ($key == $field['value']) checked @endif
								...
								@case('editor')
									...
									<div class="col-sm-9">
										...
										<div class="row row-editor">
											<div class="editor" id="{{ $field['name'] }}_editor" name="{{ $field['name'] }}_editor">
												{!! $field['value'] ?? '' !!}</div>
										</div>
									</div>
								@break;
