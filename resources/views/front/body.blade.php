...
@section('content')
	<form method="get" action="{{ route('player.body', ['question' => $step->id]) }}" enctype="multipart/form-data"
				...
				@for ($column = 0; $column < $columns; $column++)
					<div class="col-{{ $grid }}">
						<div class="@if ($column == 0) mr-4 @endif">
							<img id="image{{ $letters[$imageNo] }}" src="/uploads/{{ $qa['image' . $letters[$imageNo]] }}"
								data-id="{{ $letters[$imageNo] }}" alt="" class="step-image img-fluid">
						</div>
					</div>
					@php($imageNo++)
				@endfor
...
@push('scripts.injection')
	<script>
		...
		@if (intval($test->options) & \App\Models\TestOptions::MOUSE_TRACKING->value)
			let mousePos;
			let mouseMoves = [];
