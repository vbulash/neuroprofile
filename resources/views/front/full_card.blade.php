	...
	<form method="get" action="{{ route('player.full', ['sid' => session()->getId()]) }}">
					...
					@switch($type)
						...
						@case('phone')
							<div class="form-floating col-md-6">
								...
								<label class="form-label" for="{{ $name }}">{{ $title }}@if ($required)
										*
									@endif
								</label>
							</div>
							...
							<div class="form-group col-md-6">
								...
								<select name="{{ $name }}" id="{{ $name }}" class="select2"
									...
									@foreach ($element['cases'] as $option)
										<option value="{{ $option['value'] }}" @if ($option['value'] == $element['value']) selected @endif>
											{{ $option['label'] }}
										</option>
									@endforeach
								</select>
		...
		@if ($mix)
			<div class="form-group">
				...
				<input type="text" name="pkey" id="pkey"
					class="form-control col-lg-4 col-md-4 @error('pkey') is-invalid @enderror mt-2" required>
			</div>
		@endif
		...
		<div class="checkbox mt-2 mb-2">
			<label>
				...
				Я принимаю положения <a href="{{ route('player.policy', ['document' => 'privacy']) }}">Политики
		...
		<div class="checkbox mt-2 mb-2">
			<label>
				...
				Я согласна / согласен на обработку моих персональных данных в соответствии с <a
					href="{{ route('player.policy', ['document' => 'personal']) }}">Политикой в отношении обработки персональных
					данных</a>
			</label>
		</div>
		...
		<button type="submit" id="start_test"
			...
			@elseif ($test->options & \App\Models\TestOptions::EYE_TRACKING->value)
